...
public class CartService
{
    ...
    private Product requireProduct(String productId) throws Exception
    {
    ...
    public Set<User.ShoppingCartData> replaceCart(String token, Set<User.ShoppingCartData> newCart) {
        User user = getUserFromToken(token);
        resolveCart(user);
        user.getUserData().setShoppingCart(newCart != null ? newCart : new HashSet<>());
    ...
    public Set<User.ShoppingCartData> getGuestCart(Set<User.ShoppingCartData> cart)
    {
        return cart;
    }

    public Set<User.ShoppingCartData> addToGuestCart(Set<User.ShoppingCartData> cart, String productId, Integer quantity) throws Exception
    {
        Product product = requireProduct(productId);
    ...
    public Set<User.ShoppingCartData> increaseGuestQuantity(
            Set<User.ShoppingCartData> cart, String productId, Integer amount) throws Exception
    {
        Product product = requireProduct(productId);
    ...
    public Set<User.ShoppingCartData> decreaseGuestQuantity(Set<User.ShoppingCartData> cart, String productId, Integer amount) throws Exception
    {
        User.ShoppingCartData item = cart.stream()
                .filter(i -> i.getProductId().equals(productId))
                .findFirst()
    ...
    public Set<User.ShoppingCartData> removeFromGuestCart(Set<User.ShoppingCartData> cart, String productId) throws Exception
    {
        if (cart.isEmpty()) throw new Exception("Cart is empty");
        boolean removed = cart.removeIf(item -> item.getProductId().equals(productId));
        if (!removed) throw new Exception("Item not found in cart");
    ...
    public List<CartAction.CartProduct> getCartProductsFromCartMeta(Set<User.ShoppingCartData> cart)
    {
        return cart.stream().map(el -> {
            ...
            return CartAction.CartProduct.builder()
                ...
                .build();
        }).sorted(Comparator.comparing(CartAction.CartProduct::getName)).toList();
    }
